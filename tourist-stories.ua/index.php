...
<ul>
  ...
  <li><a href="filtr_city.php?type_trip=family"><strong>Семейный</strong></a> </li>
    ...
    $page=(int)$_GET['page'];
    $count=mysql_query("SELECT COUNT(*) FROM story where story.moderat=1 ORDER BY $sorting ",$link);
    ...
    if($temp[0]>0){
        $result=mysql_query("Select * from story, type_of_trip where story.id_type_of_story=type_of_trip.id_type_of_trip and story.moderat=1 ORDER BY $sorting LIMIT $start, $num",$link);
        if(mysql_num_rows($result)>0){
