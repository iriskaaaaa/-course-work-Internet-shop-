  ...
  <li><a href="view_type_trip.php?type_trip=family"><strong>Семейный</strong></a> </li>
      ...
      <!-- <button class="navbar-toggle collapsed menu_btn" data-toggle="collapse" data-target="#sort">
     		 ...
     		 <ul class="nav navbar-nav sort" >
                    ...
                    <?php 
                    echo'
            			<li><a href="search.php?q='.$search.'&sort=popular-acs">Возрастание</a></li>
            			<li><a href="search.php?q='.$search.'&sort=popular-desc">Убывание</a></li> ';
                        ?>
         	 		</ul>
       			 </li>
        		<li class="dropdown">
                     ...
                     <?php 
                    echo'
           				 <li><a href="search.php?q='.$search.'&sort=date-acs">Возрастание</a></li>
          				  <li><a href="search.php?q='.$search.'&sort=date-desc">Убывание</a></li> ';
                    ?>
        		    </ul>
       		   </li>
      		</ul>
    ...
    $page=(int)$_GET['page'];
    $count=mysql_query("SELECT COUNT(*) FROM story WHERE moderat=1 and name_story LIKE '%$search%'",$link);
    ...
    if($temp[0]>0){
        $result=mysql_query("Select * from story, type_of_trip where story.id_type_of_story=type_of_trip.id_type_of_trip AND story.moderat=1 and story.name_story LIKE '%$search%' ORDER BY $sorting LIMIT $start, $num",$link);
        if(mysql_num_rows($result)>0){
            $row=mysql_fetch_array($result);
            do{
...
$id=$row['id_story'];
    ...
    <a href="story.php?id_story='.$row['id_story'].'"><img src="'.$img_path.'" class="img-responsive" /></a>
   	<div class="example_text">
                ...
                <p class="glyphicon"> Вид отдыха: '.$row['name_type'].'</p></br>';
...
if ($page != 1) $pervpage = '<li><a class="pstr-prev" href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 1) .'" />&laquo;</a></li>';

if ($page != $total) $nextpage = '<li><a class="pstr-next" href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 1) .'"/>&raquo;</a></li>';

// Находим две ближайшие станицы с обоих краев, если они есть
if($page - 5 > 0) $page5left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 5) .'">'. ($page - 5) .'</a></li>';
if($page - 4 > 0) $page4left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 4) .'">'. ($page - 4) .'</a></li>';
if($page - 3 > 0) $page3left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 3) .'">'. ($page - 3) .'</a></li>';
if($page - 2 > 0) $page2left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 2) .'">'. ($page - 2) .'</a></li>';
if($page - 1 > 0) $page1left = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page - 1) .'">'. ($page - 1) .'</a></li>';

if($page + 5 <= $total) $page5right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 5) .'">'. ($page + 5) .'</a></li>';
if($page + 4 <= $total) $page4right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 4) .'">'. ($page + 4) .'</a></li>';
if($page + 3 <= $total) $page3right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 3) .'">'. ($page + 3) .'</a></li>';
if($page + 2 <= $total) $page2right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 2) .'">'. ($page + 2) .'</a></li>';
if($page + 1 <= $total) $page1right = '<li><a href="search.php?q='.$search.'&sort='.$sort.'&'.$url.'page='. ($page + 1) .'">'. ($page + 1) .'</a></li>';
