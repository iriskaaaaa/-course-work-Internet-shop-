...
$('#button-send-review').click(function(){
   ...
   var user_id = $("#button-send-review").attr("user_id");
    ...
    if ( name_review == '1' && comment_review == '1')
      {
      ...
      $.ajax({
         ...
         data: "name="+name+"&comment="+comment+"&user_id="+user_id+"&id="+iid,
         dataType: "html",
         cache: false,
         success: function() {
...
$('#button-send-pass').click(function(){
                
   var pass = $("#pass").val();
   var new_pass = $("#new_pass").val();
   var newnew_pass = $("#newnew_pass").val();
   var iid = $("#button-send-pass").attr("iid");
    ...
    if (pass != "")
     ...
     {
          name1_review = '1';
          $("#new_pass").css("borderColor","#DBDBDB");
      }else {
           name1_review = '0';
           $("#new_pass").css("borderColor","#FDB6B6");
      }
                      
    if (newnew_pass != "")
     {
          comment_review = '1';
          $("#newnew_pass").css("borderColor","#DBDBDB");
     }else {
          comment_review = '0';
          $("#newnew_pass").css("borderColor","#FDB6B6");
     } 
                                         
            
            // Глобальная проверка и отправка отзыва
            
    if ( name_review == '1' && comment_review == '1' && name1_review == '1')
      {
       if(new_pass==newnew_pass){
         $("#button-send-pass").hide();
         $("#reload-img").show();
                  
      $.ajax({
         type: "POST",
         url: "/actions/new_pass.php",
         data: "pass="+pass+"&new_pass="+new_pass+"&newnew_pass="+newnew_pass+"&id="+iid,
         dataType: "html",
         cache: false,
         success: function() {
         setTimeout("$.fancybox.close()", 1000);
